# ifndef _SERIAL_HARDDRIVER_H
#define _SERIAL_HARDDRIVER_H

#include "zigbee/HardDriver.h"
...
#include <sys/stat.h>
#include <sys/time.h>
#include <unistd.h>     //for FD_XXX()

using namespace std;

class Serial_HardDriver : public HardDriver
{
private:
  string m_device;
  unsigned int m_baudrate;
  pthread_mutex_t m_memLock;    //互斥锁
  pthread_mutex_t m_msgLock;

  int m_serial_fd;              //串口文件描述符
  ...
  bool _serialOpen(const char* dev)
  {
      ...
      m_serial_fd = open(dev, O_RDWR | O_NONBLOCK);	//打开串口文件，得到串口句柄
      if(m_serial_fd < 0)
      {
          //API_LOG(this, ERROR_LOG, "cannot open device %s\n", dev);
          //print error msg;
          cout<<"cannot open serial device\n"<<endl;
          return false;
      }
      return true;
  }
  ...
  bool _serialFlush()
  {
      if(m_serial_fd < 0)
      {
          ...
          cout<<"flushing fail because no device is opened"<<endl;
          return false;
      }
      else
      {
          tcflush(m_serial_fd, TCIFLUSH);		//刷清输入fifo缓冲区
          return true;
      }
  }
  ...
  bool _serialConfig(int baudrate, char data_bits, char parity_bits, char stop_bits)
  {
    ...
    int std_rate[] = {
      ...
      if((tcsetattr(m_serial_fd,TCSANOW,&newtio))!=0) {
          ...
          cout<<"fail to active configuration\n"<<endl;
          return false;
      }
      return true;
  }
...
public:
  Serial_HardDriver(string device, unsigned int baudrate)
  {
    m_device = device;
    m_baudrate = baudrate;
